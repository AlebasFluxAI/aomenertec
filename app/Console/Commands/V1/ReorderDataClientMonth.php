        ...
        while (true) {
            ...
            if (count($clients)>0) {
                foreach ($clients as $client) {
                    ...
                    $start_date = Carbon::create($year_aux, $month_aux, (intval($billing_day) + 1));
                    $end_date = Carbon::create($reference_date->format('Y'), $reference_date->format('m'), intval($billing_day), "23", "59", 59);
                    $data_aux = $client->dailyMicrocontrollerData()
                        ->where('year', $year_aux)
                        ->where('month', ($month_aux))
                        ->whereBetween('day', ['0' . (intval($billing_day) + 1), ($start_date->format('t'))]);
                    $data_month = $client->dailyMicrocontrollerData()
                        ->where('year', $reference_date->format('Y'))
                        ->where('month', $reference_date->format('m'))
                        ->whereBetween('day', ['01', intval($billing_day)])
                        ->union($data_aux)
                        ->get();
