            ...
            <tbody>
                ...
                @isset($table_rows)
                    @foreach($table_rows as $index=>$table_row)
                        <div wire:key="equipment-field-{{ $index }}" class="form-group mb-2 align-content-start col-md-3 col-sm-3">
                        <tr class="shadow-sm">
                            @foreach($table_headers as $header_name=>$table_header)
                                <td>
                                    ...
                                    @elseif($header_name == "Serial")
                                        <div class="input-group">
                                            <input wire:model="equipment.{{ $index }}.serial"
                                                   wire:keydown.enter="assignEquipmentFirst({{$table_row['type_id']}})" type="number" class="form-control" id="equipment.{{ $index }}.serial" autocomplete="off" placeholder="{{ $table_row['type'] }}" required >
